...
use crate::source::postgres::{metrics::PgSourceMetrics, DefiniteError};
...
pub(crate) fn render<G: Scope<Timestamp = MzOffset>>(
    ...
    let (button, errors) = builder.build_fallible(move |caps| {
        ...
        Box::pin(async move {
            ...
            let mut client = connection_config.connect_replication().await?;

            let min_resume_lsn = super::ensure_replication_slot(&client, &connection).await?;
            assert!(resume_lsn == MzOffset::from(0) || min_resume_lsn <= resume_lsn, "slot compacted past resumption point. resume_lsn={resume_lsn} min_resume_lsn={min_resume_lsn}");
            ...
            while let Some(event) = rewind_input.next_mut().await {
                if let AsyncEvent::Data(cap, data) = event {
                    ...
                    for req in data.drain(..) {
                        assert!(min_resume_lsn <= req.snapshot_lsn, "slot compacted past snapshot point. snapshot_lsn={} min_resume_lsn={min_resume_lsn}", req.snapshot_lsn);
            ...
            loop {
                // TODO(guswynn|petrosagg): fix the underlying bug that prevents client re-use when
                // exiting the CopyBoth mode, so we don't need to re-create clients in this function.
                client = connection_config.connect("advance_upper").await?;

                if let Err(err) = ensure_publication_exists(&client, &connection.publication).await? {
                    // If the publication gets deleted there is nothing else to do
                ...
                // is currently empty. This unblocks downstream consumers immediately instead of
                // making them wait for TX_TIMEOUT to trigger.
                ...
                client = connection_config.connect_replication().await?;
                ...
                while let Ok(_) = tokio::time::timeout(TX_TIMEOUT, stream.as_mut().peek()).await {
                    ...
                    while let Some(event) = stream.as_mut().peek().now_or_never() {
                        match event {
                            ...
                            Some(Ok(_)) => {
                                bail!("received replication event outside of transaction")
                            }
                            Some(Err(_)) => {
...
fn raw_stream<'a>(
...
) -> impl AsyncStream<Item = Result<LogicalReplicationMessage, anyhow::Error>> + 'a {
    async_stream::try_stream!({
        ...
        let resume_lsn = PgLsn::from(resume_lsn.offset.saturating_sub(1));
        let query = format!(
        ...
        loop {
            let send_feedback = match select(stream.as_mut().next(), uppers.as_mut().next()).await {
                future::Either::Left((next_message, _)) => match next_message.transpose()? {
                    ...
                    Some(_) => Err(anyhow!("unexpected replication message"))?,
                    None => return,
                },
...
fn extract_transaction<'a>(
    stream: impl AsyncStream<Item = Result<LogicalReplicationMessage, anyhow::Error>> + 'a,
    ...
    Item = Result<(u32, Result<Vec<Option<Bytes>>, DefiniteError>, Diff), anyhow::Error>,
> + 'a {
    ...
    async_stream::try_stream!({
        ...
        let commit_lsn = match stream.try_next().await? {
            ...
            Some(_) => Err(anyhow!("transaction begun without BEGIN"))?,
            None => Err(anyhow!("stream ended prematurely"))?,
        };
            ...
            match event {
                ...
                Relation(body) => {
                    ...
                    if let Some((_, expected_desc, _)) = table_info.get(&rel_id) {
                        ...
                        match current_info.iter().find(|t| t.oid == rel_id) {
                            Some(current_desc) => {
                            ...
                            None => {
                                errored_tables.insert(rel_id);
                                yield (rel_id, Err(DefiniteError::TableDropped), 1);
                            }
                        }
                    }
                }
                Truncate(body) => {
                    for &rel_id in body.rel_ids() {
                        if errored_tables.contains(&rel_id) {
                            continue;
                        }
                        errored_tables.insert(rel_id);
                ...
                Commit(body) => {
                    if commit_lsn != body.commit_lsn() {
                        Err(anyhow!("lsn mismatch between BEGIN and COMMIT"))?
                    }
                    return;
                }
                ...
                Begin(_) => Err(anyhow!("BEGIN within existing BEGIN stream"))?,
                // The enum is marked as non_exhaustive. Better to be conservative
                _ => Err(anyhow!("unexpected logical replication message"))?,
            }
        }
        Err(anyhow!("stream ended prematurely"))?;
    })
}
...
async fn ensure_publication_exists(
    client: &Client,
    publication: &str,
) -> Result<Result<(), DefiniteError>, anyhow::Error> {
    // Figure out the last written LSN and then add one to convert it into an upper.
    let result = client
        .query_opt(

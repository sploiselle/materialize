...
impl SourceRender for PostgresSourceConnection {
    ...
    fn render<G: Scope<Timestamp = MzOffset>>(
        ...
        for SourceExport { ingestion_output: reference, .. } in config.source_exports.values() {
            ...
            let casts = self.table_casts[reference].clone();
            table_info.insert(desc.oid, (*reference, desc.clone(), casts.clone()));
        }

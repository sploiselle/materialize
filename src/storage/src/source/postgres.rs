...
//!
//! Transient errors (represented as `anyhow::Error`) are any errors that can happen for reasons
//! that are unrelated to the data itself. This could be authentication failures, connection
//! failures, etc. The only operators that can emit such errors are the `TableReader` and the
//! `ReplicationReader` operators, which are the ones that talk to the external world. Both of
//! these operators are built with the `AsyncOperatorBuilder::build_fallible` method which allows
//! transient errors to be propagated upwards with the standard `?` operator without risking
//! downgrading the capability and producing bogus frontiers.
//!
...
impl SourceRender for PostgresSourceConnection {
    ...
    fn render<G: Scope<Timestamp = MzOffset>>(
        ...
        for (i, desc) in self.publication_details.tables.iter().enumerate() {
            ...
            // We maintain descriptions for all tables in the publication, but only casts for those
            // we aim to use (and have validated that their types). This prevents us from creating
            // snapshots for tables in the publication that are not referenced in the source.
...
/// A definite errors that can end up in the collection of a specific table.
...
/// Ensures the replication slot of this connection is created and returns the minimum LSN at which
/// this slot can safely resume.
async fn ensure_replication_slot(
    client: &Client,
    connection: &PostgresSourceConnection,
) -> Result<MzOffset, anyhow::Error> {
    ...
    match query_single_row(client, &create_slot).await {
        Ok(row) => {
            // For postgres, `consistent_point` means that the slot is able to produce all
            // transactions that happen at tx_lsn > consistent_point. Therefore the upper is
            // consistent_point + 1
            let min_lsn: PgLsn = row.get("consistent_point").unwrap().parse().unwrap();
            Ok(MzOffset::from(min_lsn) + 1)
        }
        // If the slot already exists discover its minimum resume LSN
        Err(SingleRowError::Other(err)) if err.code() == Some(&SqlState::DUPLICATE_OBJECT) => {
            loop {
                let query = format!(
                    "SELECT confirmed_flush_lsn FROM pg_replication_slots WHERE slot_name = '{slot}'"
                );
                let Some(row) = query_single_row_opt(client, &query).await? else {
                    bail!("replication slot mysteriously missing");
                };
                ...
                match row.get("confirmed_flush_lsn") {
                    ...
                    Some(flush_lsn) => {
                        return Ok(MzOffset::from(flush_lsn.parse::<PgLsn>().unwrap()) + 1)
                    }
        ...
        Err(err) => bail!("error creating replication slot {err:#}"),
    }
}

/// Runs the given query using the client and expects at most a single row to be returned.
async fn query_single_row_opt(
    client: &Client,
    query: &str,
) -> Result<Option<SimpleQueryRow>, SingleRowError> {
    ...
    match (rows.next(), rows.next()) {
        ...
        (None, None) => Ok(None),
        _ => Err(SingleRowError::MultipleRows),
    }
}

/// Runs the given query using the client and expects exactly one row to be returned.
async fn query_single_row(client: &Client, query: &str) -> Result<SimpleQueryRow, SingleRowError> {
    match query_single_row_opt(client, query).await? {
        Some(row) => Ok(row),
        _ => Err(SingleRowError::NoRows),
    }
}

/// Errors that can occur when executing a single row query.
#[derive(Debug, thiserror::Error)]
enum SingleRowError {
    /// The action can only be performed by a superuser.
    #[error("error executing query: {0}")]
    Other(#[from] tokio_postgres::Error),

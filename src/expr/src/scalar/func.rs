...
impl NullaryFunc {
    pub fn output_type(&self) -> ColumnType {
        match self {
            NullaryFunc::MzLogicalTimestamp => ScalarType::APD { scale: None }.nullable(false),
        }
    }
}
...
fn power_apd<'a>(a: Datum<'a>, b: Datum<'a>) -> Result<Datum<'a>, EvalError> {
    ...
    if a.is_zero() {
        if b.is_zero() {
            return Ok(Datum::from(Apd::from(1)));
        }
        if b.is_negative() {
            return Err(EvalError::Undefined(
                "zero raised to a negative power".to_owned(),
...
impl UnaryFunc {
    ...
    pub fn output_type(&self, input_type: ColumnType) -> ColumnType {
        ...
        match self {
            ...
            CastFloat64ToFloat32 | CastInt32ToFloat32 | CastInt64ToFloat32 | CastAPDToFloat32 => {
                ScalarType::Float32.nullable(in_nullable)
            }

            CastInt32ToFloat64 | CastInt64ToFloat64 | CastFloat32ToFloat64 | CastAPDToFloat64 => {
                ScalarType::Float64.nullable(in_nullable)
            }

            CastInt64ToInt32 | CastAPDToInt32 => ScalarType::Int32.nullable(in_nullable),

            CastFloat32ToInt32 | CastFloat64ToInt32 => ScalarType::Int32.nullable(true),

            CastInt32ToInt64 | CastFloat32ToInt64 | CastFloat64ToInt64 | CastAPDToInt64 => {

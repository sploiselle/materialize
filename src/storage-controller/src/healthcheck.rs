    ...
    T: Timestamp + Lattice + Codec64 + TimestampManipulation + From<EpochMillis>,
{
        ...
        I: IntoIterator<Item = Row>,
    {
        ...
        for row in previous_statuses {
            ...
            // book keeping or placing it in memory.
    ...
    fn should_produce_row(&mut self, type_: IntrospectionType, new: &Row) -> bool {
        ...
        let produce = match self.previous_statuses.get(&this_id) {
            None => true,
            Some(prev) => {
                let prev_val = prev.iter().next().expect("prev value always one datum");
                (type_.produce_new_value)(prev_val, new_val)
            }
        };

        // Remember the last row we produced to allow envelope-like behavior.
        if produce {
            let mut row = self.row_buf.packer();
            row.push(new_val);
            self.previous_statuses.insert(this_id, self.row_buf.clone());
        }

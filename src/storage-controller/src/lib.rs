    ...
    StorageResponse<T>: RustType<ProtoStorageResponse>,
{
    ...
    async fn create_collections(
        ...
        for (id, description, write, since_handle, metadata) in to_register {
            ...
            match description.data_source {
                ...
                DataSource::IngestionExport {
                    ...
                    let source_collection =
                        self.collection_mut(ingestion_id).expect("known to exist");
                    match &mut source_collection.description {
                        CollectionDescription {
                            ...
                            let output_index = ingestion_desc
                                ...
                                .expect("source desc contains all external references");
            ...
            self.determine_collection_dependencies(id)?;
        }
    ...
    async fn alter_ingestion_source_desc(
        ...
        for (id, desc) in collections {
            ...
            if curr_ingestion.desc != desc {
                let mut source_exports = BTreeMap::from_iter([(
                    id,
                    SourceExport {
                        output_index: 0,
                        storage_metadata: (),
                ...
                // Get the updated output indices for each.
                //
                ...
                // looked up its output index index when we were aligning the
                // rendering outputs.
                for export_id in curr_ingestion.source_exports.keys() {
                    ...
                    let external_ref = match &self.collection(*export_id)?.description.data_source {
                        DataSource::IngestionExport {
                            ingestion_id: _,
                            external_reference,
                        } => external_reference,
                        _ => unreachable!("source exports must be DataSource::SourceExport"),
                    };

                    let output_index = match desc.connection.output_idx_for_name(external_ref) {
                        Some(idx) => idx,
                        // metadatadata describing this export has been dropped.
                        None => continue,
                    };
            ...
            if let DataSource::IngestionExport { ingestion_id, .. } = dropped_data_source {
                ...
                match &mut ingestion_collection.description {
                    CollectionDescription {
                        ...
                        // It's possible that updating the ingestion description
                        // has already removed this export, in which case
                        // nothing changed and we don't need to re-execute the
                        // ingestion.
                        if removed.is_some() {
                            to_execute.insert(ingestion_id);
                        }
    ...
    Self: StorageController<Timestamp = T>,
{
    ...
    /// Determines a collection's dependencies and installs any appropriate read
    /// holds.
    ///
    /// Note that this might include fast-forwarding new collections' sinces as
    /// far as their dependencies' sinces.
    fn determine_collection_dependencies(&mut self, id: GlobalId) -> Result<(), StorageError<T>> {
        ...
        let storage_dependency = match &collection.description.data_source {
            ...
            DataSource::IngestionExport { ingestion_id, .. } => {
                match &self.collection(*ingestion_id)?.description.data_source {
                    DataSource::Ingestion(ingestion) => ingestion.remap_collection_id,
        ...
        if PartialOrder::less_than(&collection.implied_capability, &dependency_since) {
            // Fast-forward the collection's since as far forward as its
            // dependency. This is necessary so that we can install a read
            // capability on the dependency––i.e. the dependency's since can
            // never be in advance of its dependent's sinces.
    ...
    /// Schedule the identified collections to run.
    ///
    /// # Panics
    /// - If any collction's description is `DataSource::IngestionExport`; those
    ///   collections should not be executed directly. Instead, the ingestion
    ///   they reference should be executed only after the export has been added
    ///   to them.
    /// - Other collection types have other specific invariants they hold which
    ///   may produce panics.

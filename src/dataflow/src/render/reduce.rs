...
use repr::adt::apd::{self, APD_AGG_WIDTH, APD_DATUM_WIDTH};
...
enum AccumInner {
    ...
    APD {
        ...
        accum: OrderedDecimal<DecNum<APD_AGG_WIDTH>>,
        /// Counts +inf
        pos_infs: isize,
        /// Counts -inf
        neg_infs: isize,
    ...
    fn plus_equals(&mut self, other: &AccumInner) {
        match (&mut *self, other) {
            ...
            (
                ...
                assert!(!cx_w.status().rounded(), "AccumInner::APD overflow");
    ...
    fn multiply(self, factor: &isize) -> AccumInner {
        ...
        match self {
            ...
            AccumInner::APD {
                ...
                let mut f = DecNum::<APD_AGG_WIDTH>::from(factor);
        ...
        .map(|f| {
            let inner = match f.func {
                ...
                AggregateFunc::SumAPD => AccumInner::APD {
                    accum: OrderedDecimal(DecNum::<APD_AGG_WIDTH>::zero()),
                    pos_infs: 0,
                    neg_infs: 0,
                    nans: 0,
                    non_nulls: 0,
            ...
            AggregateFunc::SumAPD => match datum {
                Datum::APD(n) => {
                    let (accum, pos_infs, neg_infs, nans) = if n.0.is_infinite() {
                        if n.0.is_negative() {
                            (DecNum::<APD_AGG_WIDTH>::zero(), 0, 1, 0)
                        } else {
                            (DecNum::<APD_AGG_WIDTH>::zero(), 1, 0, 0)
                        }
                    } else if n.0.is_nan() {
                        (DecNum::<APD_AGG_WIDTH>::zero(), 0, 0, 1)
                    } else {
                        ...
                        match (&aggr.func, &accum.inner) {
                            ...
                            (
                                ...
                                if *nans > 0 || (pos_inf && neg_inf) {
                                    Datum::APD(OrderedDecimal(DecNum::<APD_DATUM_WIDTH>::nan()))
                                } else if pos_inf {
                                    Datum::APD(
                                        OrderedDecimal(DecNum::<APD_DATUM_WIDTH>::infinity()),
                                    )
                                } else if neg_inf {
                                    Datum::APD(OrderedDecimal(
                                        -DecNum::<APD_DATUM_WIDTH>::infinity(),
                                    ))
                                } else if *non_nulls == 0 {
                                    Datum::Null
                                } else {

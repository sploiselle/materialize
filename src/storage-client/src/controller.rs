    ...
    Self: StorageController<Timestamp = T>,
{
    ...
    async fn advance_uninitialized_collection_since<I: IntoIterator<Item = GlobalId>>(
        ...
        for id in ids {
            // Fast-forward our implied capability; this is the since the collection
            // would have been at if it were created alongside the other subsources
            // initially.
            let collection = self.collection_mut(id).expect("validated exists");
            assert_eq!(
            ...
            // Persist that the shard should be advanced to the dependency's since.

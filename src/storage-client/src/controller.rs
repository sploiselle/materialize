    ...
    DurableExportMetadata<T>: mz_stash::Data,
{
        ...
        for (id, desc) in &collections {
            let remap_shard = match &desc.data_source {
                ...
                DataSource::Ingestion(IngestionDescription {
                    remap_collection_id,
                    ..
                }) => Some(match self.collection(*remap_collection_id) {
                    Ok(c) => c.collection_metadata.data_shard,
                    Err(_) => entries
                        .iter()
                        .find(|(id, _)| id == remap_collection_id)
                        .map(|(_, DurableCollectionMetadata { data_shard, .. })| *data_shard)
                        .expect("progress collection must exist"),
                }),
    ...
    fn update_read_capabilities(
        ...
        let mut dependent_updates = BTreeMap::new();
        ...
        while let Some(key) = updates.keys().rev().next().cloned() {
            ...
            if let Ok(collection) = self.collection_mut(key) {
                ...
                if let Some(id) = collection.since_dependent {
                    // Only mirror changes that do not close the collection;
                    // dependency represents an upper bound, so outliving the
                    // dependency is fine.
                    if !collection.read_capabilities.frontier().is_empty() {
                        let cur = dependent_updates
                            .entry(id)
                            .or_insert_with(|| Antichain::from_elem(T::minimum()));

                        let dependency_frontier =
                            collection.read_capabilities.frontier().to_owned();
                        if PartialOrder::less_than(cur, &dependency_frontier) {
                            *cur = dependency_frontier;
                        }
                    }
                }
        ...
        // Set read policy for dependent collections to be the maximum frontier
        // of the dependency's read frontier. The invariant we are maintaining
        // here is that the remap collection's since never meets or exceeds the
        // data collection's upper, so performing this work after downgrading
        // the data collection's since will not break that invariant.
        self.set_read_policy(
            dependent_updates
                .into_iter()
                .map(|(id, max_frontier)| (id, ReadPolicy::ValidFrom(max_frontier)))
                .collect(),
        );
    }
    ...
    Self: StorageController<Timestamp = T>,
{
    ...
    /// on-disk and `current_metadata` as its in-memory representation) to
    /// instead be that of `metadata_to_update`.
    ///
    ...
    async fn rewrite_collection_metadata(
        ...
        for (id, new_metadata) in metadata_to_update.iter() {
            let metadata = &current_metadata[id];
            ...
            current_metadata.insert(*id, new_metadata.clone());
        }
        ...
        METADATA_COLLECTION
            .upsert(&mut self.state.stash, metadata_to_update.into_iter())
            .await
        ...
        for id in remap_shards_to_replace {
            ...
            c.collection_metadata.remap_shard = current_metadata[&id].remap_shard;
        }
        ...
        for id in data_shards_to_replace {
            ...
            let data_shard = current_metadata[&id].data_shard;
...
pub struct CollectionState<T> {
    ...
    ///
    /// What motivates this feature is ensuring that a remap collection's since
    /// never meets or exceeds the data collection's upper.

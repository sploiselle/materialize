...
impl CatalogItem {
    ...
    /// Rewrites `self.create_sql`, replacing all instances of `from` with `to`,
    /// or errors if the operation is too ambiguous for us to resolve.
        ...
        for op in ops {
            let mut ops = match op {
                ...
                Op::RenameItem { id, to_name } => {
                    ...
                    let item = match entry.item.rewrite_create_sql(&entry.name, &to_full_name) {
                        Ok(i) => i,
                        Err(e) => {
                    ...
                    for id in entry.used_by() {
                        ...
                        let updated_item = match dependent_item
                            .item
                            .rewrite_create_sql(&entry.name, &to_full_name)
                        {
                            Ok(i) => i,
                            Err(e) => {
                                return Err(Error::new(ErrorKind::AmbiguousRename(format!(

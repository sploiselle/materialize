...
impl SessionVars {
    pub fn new(build_info: &'static BuildInfo, user: User) -> SessionVars {
        ...
        s.with_var(&APPLICATION_NAME)
            .with_constrained_var(&CLIENT_ENCODING, ValueConstraint::Fixed)
            .with_var(&CLIENT_MIN_MESSAGES)
            .with_var(&CLUSTER)
            .with_var(&CLUSTER_REPLICA)
            .with_var(&DATABASE)
            .with_constrained_var(&DATE_STYLE, ValueConstraint::Fixed)
            .with_var(&EXTRA_FLOAT_DIGITS)
            .with_var(&FAILPOINTS)
            .with_constrained_var(&INTEGER_DATETIMES, ValueConstraint::Fixed)
            .with_constrained_var(&INTERVAL_STYLE, ValueConstraint::Fixed)
            .with_var(&SEARCH_PATH)
            .with_constrained_var(&SERVER_VERSION, ValueConstraint::ReadOnly)
            .with_constrained_var(&SERVER_VERSION_NUM, ValueConstraint::ReadOnly)
            .with_var(&SEARCH_PATH)
            .with_var(&SQL_SAFE_UPDATES)
            .with_constrained_var(&STANDARD_CONFORMING_STRINGS, ValueConstraint::Fixed)
            .with_var(&STATEMENT_TIMEOUT)
            .with_var(&IDLE_IN_TRANSACTION_SESSION_TIMEOUT)
            .with_var(&TIMEZONE)
            .with_var(&TRANSACTION_ISOLATION)
            .with_var(&REAL_TIME_RECENCY)
    ...
    {
        self.vars
            .insert(var.name, Box::new(SessionVar::new(var).with_constraint(c)));
        self
    }
    ...
    pub fn notify_set(&self) -> impl Iterator<Item = &dyn Var> {
        ...
        .into_iter()
        .map(|p| self.get(p.name()).expect("SystemVars known to exist"))
    ...
    pub fn reset(&mut self, name: &str, local: bool) -> Result<(), VarError> {
        let name = UncasedStr::new(name);
        if name == MZ_VERSION_NAME || name == IS_SUPERUSER_NAME {
            Ok(())
        } else {
            self.vars
                .get_mut(name)
    ...
    pub fn set_cluster(&mut self, cluster: String) {
        self.set(CLUSTER.name(), VarInput::Flat(&cluster), false)
    ...
    V::Owned: Debug + Send + Sync,
{
    ...
    fn with_constraint(mut self, c: ValueConstraint<V>) -> SessionVar<V> {
        assert!(
            !self
                .constraints
                .iter()

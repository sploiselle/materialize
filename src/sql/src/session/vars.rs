...
impl SessionVars {
    ...
    ///
    /// The new value may be either committed or rolled back by the next call to
    /// [`SessionVars::end_transaction`]. If `local` is true, the new value is always discarded by
    /// the next call to [`SessionVars::end_transaction`], even if the transaction is marked to
    /// commit.
    ///
    /// Like with [`SessionVars::get`], configuration parameters are matched case insensitively. If
    /// the named configuration parameter does not exist, an error is returned.
    ///
    /// If the variable does not exist or the user does not have the visibility requires, this
    /// function returns an error.
    pub fn reset(

...
static REAL_TIME_RECENCY: ServerVar<bool> = ServerVar {
    ...
    internal: true,
    feature_flag_gate: Some(&ALLOW_REAL_TIME_RECENCY),
};
    ...
    V: fmt::Debug + ?Sized + 'static,
{
    ...
    internal: bool,
    feature_flag_gate: Option<&'static FeatureFlag>,
}
    ...
    V: Value + fmt::Debug + ?Sized + 'static,
{
    ...
    fn can_be_set(&self, system_vars: Option<&SystemVars>) -> Result<(), VarError> {
        ...
        if let Some(gate) = self.feature_flag_gate {
            gate.enabled(system_vars)?;
        }

...
pub fn plan_homogeneous_exprs(
    ...
    // `.or(type_hint)` is necessary, e.g. to plan all-parameter exprs where we
    // cannot determine a common type.
    let target = match cast::determine_best_common_type(&types).or(type_hint) {
        Some(t) => t,

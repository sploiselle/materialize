...
pub fn plan_delete_query(
    ...
    plan_mutation_query_inner(qcx, table, using, vec![], selection)
}
...
pub fn plan_update_query(
    ...
    plan_mutation_query_inner(qcx, table, vec![], assignments, selection)
}
...
pub fn plan_mutation_query_inner(
    ...
    let (id, desc, mut get, scope) = match table {
        ...
        _ => panic!("mutation queries only support tables"),
    };
...
fn handle_mutation_using_clause(
    ...
    if let Some(expr) = selection {
        // Join `FROM` with `USING` tables, like `FROM, USING...`. This gives us
        // PG-like semantics e.g. expressing ambiguous column references. We put
        // `FROM` first for no real reason, but making a different decision
        ...
        // Rewrite all column referring to `FROM` to instead be correlated to
        // the outer relation. This also means shifting all references to the
        // `USING` tables left by the number of columns in `FROM`.
        let get_arity = qcx.relation_type(&get).arity();
        expr.visit_mut(&mut |e| {
            if let HirScalarExpr::Column(c) = e {
                if c.column < get_arity {
                    c.level += 1;
                } else {
                    c.column -= get_arity;
                };
        ...
        // this filters the `USING` tables, _not_ the joined `FROM, USING...`
        // relation.

...
// Mocks out behavior like `Into<Operation<R>>` for each func type, as well as
// pass-throughs to enable macro use.
pub trait IntoOperation<R> {
    fn into_operation(self) -> Operation<R>;
}

impl IntoOperation<ScalarExpr> for Operation<ScalarExpr> {
    fn into_operation(self) -> Operation<ScalarExpr> {
        self
    }
}

impl IntoOperation<(ScalarExpr, AggregateFunc)> for Operation<(ScalarExpr, AggregateFunc)> {
    fn into_operation(self) -> Operation<(ScalarExpr, AggregateFunc)> {
        self
    }
}

impl IntoOperation<TableFuncPlan> for Operation<TableFuncPlan> {
    fn into_operation(self) -> Operation<TableFuncPlan> {
        self
    }
}

impl IntoOperation<ScalarExpr> for UnaryFunc {
    fn into_operation(self) -> Operation<ScalarExpr> {
        unary_op(move |_ecx, e| Ok(e.call_unary(self.clone())))
    }
}

impl IntoOperation<ScalarExpr> for BinaryFunc {
    fn into_operation(self) -> Operation<ScalarExpr> {
        binary_op(move |_ecx, left, right| Ok(left.call_binary(right, self.clone())))
    }
}

impl IntoOperation<ScalarExpr> for VariadicFunc {
    fn into_operation(self) -> Operation<ScalarExpr> {
...
impl IntoOperation<(ScalarExpr, AggregateFunc)> for AggregateFunc {
    fn into_operation(self) -> Operation<(ScalarExpr, AggregateFunc)> {

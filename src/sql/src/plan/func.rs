...
impl ParamList {
    ...
    ///   embdedded type, which we'll refer to as its element.
    ///
    ...
    /// e.g., can be `None` when you have an otherwise-anonymous complex values
    /// that embeds a custom element type.
    ///
    /// If we encounter a custom composite value with a custom element value,
    /// and we later encounter an otherwise-anonymous composite type with the
    /// same custom type element, we fail the function rather than "promoting"
    /// the anonymous composite type to the custom type. Why? It's hard to know
    /// how these types will be used and it's better to be more restrictive now
    /// and ease the restrictions later than vice versa. It's also trivial for
    /// users to add a cast to ensure custom type consistency. To make this
    ...
    /// /* Errors because we won't promote int4_list_custom list to
        ...
        for (i, typ) in typs.iter().enumerate() {
            ...
            match (param, typ) {
                ...
                | (
                    ...
                    // interior `ScalarType` because of

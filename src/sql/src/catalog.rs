    ...
    pub fn resolve(
        ...
        let schema = match &name.schema {
            ...
            None => match schemas.keys().exactly_one() {
                Ok(schema) => schema,
                Err(_) => {
                    bail!("table {name} is ambiguous, consider specifying the schema")
                }
            },
        };
        ...
        let database = match &name.database {
            ...
            None => match databases.keys().exactly_one() {
                ...
                Err(_) => {
                    bail!("table {name} is ambiguous, consider specifying the database")
                }
            },
        };

...
pub(super) async fn derive_catalog_from_publication_tables<'a>(
    database: &'a str,
...
pub(super) fn derive_output_to_name_map_from_publication_tables(
    database: &str,
    publication_tables: &[PostgresTableDesc],
) -> BTreeMap<usize, UnresolvedItemName> {
    // An index from table name -> schema name -> database name -> PostgresTableDesc
...
pub(super) fn derive_name_to_output_map_from_publication_tables(
    database: &str,
    publication_tables: &[PostgresTableDesc],
) -> BTreeMap<UnresolvedItemName, usize> {
    // An index from table name -> schema name -> database name -> PostgresTableDesc
    let mut output_idx_to_name = BTreeMap::new();
    for (i, table) in publication_tables.iter().enumerate() {
        output_idx_to_name.insert(
            UnresolvedItemName(vec![
                database.clone().into(),
                table.namespace.clone().into(),
                table.name.clone().into(),
...
pub(super) async fn validate_requested_subsources(
    config: &Config,
    r: &[(UnresolvedItemName, UnresolvedItemName, &PostgresTableDesc)],
) -> Result<(), PlanError> {
    ...
    if let Some(name) = r
        ...
        .cloned()
    {
        let mut upstream_references: Vec<_> = r
            .into_iter()
    ...
    let tables_to_check_permissions = r
        .iter()

...
impl ParamList {
    ...
    fn matches_argtypes(&self, ecx: &ExprContext, typs: &[Option<ScalarType>]) -> bool {
        ...
        PolymorphicSolution::new(ecx, typs, &self).is_ok()
    }
        ...
        i.iter()
            .map(|p| {
                CoercibleScalarExpr::Coerced(HirScalarExpr::literal_null(match p {
                    ...
                    o => unreachable!(
                        "o {:?} represents a pseudo type and doesn't have a ScalarType",
                        o
                    ),
                }))
            })
    ...
    fn from(s: ScalarBaseType) -> ParamType {
        ...
        let s = match s {
            Array | List | Map | Record => {
                panic!("use polymoprhic parameters rather than {:?}", s);
            }
...
enum PolymorphicCompatClass {
    ...
    /// > then the common type is required to be a non-array type. Once a
    /// > common type is identified, arguments in anycompatible and
    ...
    ///
    /// See https://www.postgresql.org/docs/current/extend-type-system.html#EXTEND-TYPES-POLYMORPHIC
    BestCommonAny,
...
impl PolymorphicSolution {
    fn new(
        ...
        params: &ParamList,
    ) -> Result<PolymorphicSolution, PlanError> {
    ...
    /// called.
    fn determine_key(&mut self, ecx: &ExprContext) -> Result<(), PlanError> {

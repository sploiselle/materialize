    ...
    for config in allowed_configs {
        match with_options.remove(&config.to_string()) {
            Some(Value::SingleQuotedString(v)) => {
                specified_options.push((config.replace("_", "."), v));
            }
...
/// with the specified options, and test its ability to create an
/// [`rdkafka::consumer::BaseConsumer`](https://docs.rs/rdkafka/latest/rdkafka/consumer/base_consumer/struct.BaseConsumer.html).
///
/// # Arguments
///
/// - `options` should be slice of tuples wit the structure `(setting name,
...
/// # Errors
///
/// - `librdkafka` cannot create a BaseConsumer using the value specified. For
///   example, when using Kerberos auth, and the provided values are invalid.

    ...
    E: fmt::Display,
{
    ...
    // Simplifies calls to lexing subprocesses by handling error
    macro_rules! lex_buf {
        ($lexer:expr) => {
    ...
    // Simplifies calls to closure by handling error
    ...
    loop {
        ...
        let elem = match buf.peek() {
            Some('"') => gen(lex_buf!(lex_list_quoted_element)?)?,
            Some('{') => {
                ...
                gen(lex_buf!(lex_list_embedded_list)?)?
            }
            Some(_) => {
                let elem = lex_buf!(lex_list_unquoted_element)?;
...
fn lex_list_quoted_element<'a>(buf: &mut LexBuf<'a>) -> Result<Cow<'a, str>, &'static str> {
    assert!(buf.consume('"'));
    let s = buf.take_while(|ch| !matches!(ch, '"' | '\\'));

    // Optimization for quoted strings without escapes.
...
fn lex_list_embedded_list<'a>(buf: &mut LexBuf<'a>) -> Result<Cow<'a, str>, &'static str> {
...
fn lex_list_unquoted_element<'a>(buf: &mut LexBuf<'a>) -> Result<Cow<'a, str>, Cow<'a, str>> {
    ...
    // Optimization for unqoted elements without special characters.
    match buf.peek() {
    ...
    let mut last_escaped_space = None;
    loop {
        match buf.next() {
            Some('\\') => match buf.next() {
                Some(c) => {
    ...
    Ok(match last_escaped_space {
        Some(escaped_space) if escaped_space > s.trim_end().len() => {
            String::from_iter(s.chars().take(escaped_space)).into()
        }

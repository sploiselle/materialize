...
use crate::r#impl::encoding::{SerdeHollowBatchReaderMetadata, SerdeReaderEnrichedHollowBatch};
...
/// Propagates metadata from readers to a `HollowBatch` to apply the desired
/// semantics.
...
where
    ...
    D: Semigroup + Codec64,
{
    snapshot_batches: Vec<ReaderEnrichedHollowBatch<T>>,
    ...
    D: Semigroup + Codec64,
{
    ...
    pub async fn next(&mut self) -> ReaderEnrichedHollowBatch<T> {
        ...
        match self.snapshot_batches.pop() {
            Some(batch) => batch,
    ...
    D: Semigroup + Codec64 + Ord,
{
    ...
    pub async fn fetch_batches(
        ...
        for batch in batches {
            let mut next = self.fetch_batch(batch).await.expect("valid usage");
            ret.append(&mut next);
        }
        ret.sort();
        ret
...
mod tests {
    ...
    async fn skip_consensus_fetch_optimization() {
        ...
        assert_eq!(read.fetch_batches(snapshot).await, all_ok(&data, 2));
        assert_eq!(

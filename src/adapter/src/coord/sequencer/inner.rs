    ...
    pub(super) async fn sequence_alter_source(
        ...
        match action {
            ...
            AlterSourceAction::AddSubsourceExports {
                ...
                // Resolve items in statement
                let (mut create_source_stmt, depends_on) = dbg!(create_sql_to_stmt_deps(
                    self,
                    ALTER_SOURCE,
                    cur_entry.create_sql()
                ))?;
                    ...
                    .ok_or(purification_err())?
                {
                    ReferencedSubsources::SubsetTables(c) => {
                        mz_ore::soft_assert!({
                            ...
                            dbg!(current_references)
                                .intersection(dbg!(&subsources))
                                .next()
                                .is_none()
                        });
                ...
                // Re-define our source in terms of the amended statement
                let plan = match dbg!(mz_sql::plan::plan(
                    None,
                    &catalog,
                    Statement::CreateSource(create_source_stmt),
                    &Params::empty(),
                ...
                // TODO create_source_inner takes CreateSourcePlans
                let CreateSourceInner {
                    mut ops,
                    sources,
                    if_not_exists_ids,

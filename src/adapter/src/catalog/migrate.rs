...
fn subsource_rewrite_v0_96(
        ...
        let tables: Vec<_> = match &desc.connection {
            ...
            GenericSourceConnection::LoadGenerator(load_generator) => {
                ...
                let prefix = UnresolvedItemName(vec![
                    Ident::new_unchecked("mz_load_generators"),
                    Ident::new_unchecked(match load_generator.load_generator {
                        LoadGenerator::Counter { .. } => "counter".to_string(),
    ...
    struct IdUpdater<'a> {
        id_map: &'a BTreeMap<GlobalId, GlobalId>,

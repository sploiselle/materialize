...
def workflow_default(c: Composition, parser: WorkflowArgumentParser) -> None:
    ...
    with c.override(testdrive, materialized):
        ...
        if args.replicas > 1:
            ...
            # Ensure we have a single-replica cluster for use with storage items
            c.sql(
                f"""
                CREATE CLUSTER single_replica_cluster SIZE = '{materialized.default_replica_size}';
                GRANT ALL PRIVILEGES ON CLUSTER single_replica_cluster TO materialize;
                """,
                user="mz_system",
                port=6877,
            )
            single_replica_cluster = "single_replica_cluster"
        else:
